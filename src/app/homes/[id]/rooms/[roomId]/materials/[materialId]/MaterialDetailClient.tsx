    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .edit-btn { background: #fff; color: #374151; border: 1.5px solid #e9edf2; padding: 8px 18px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; font-family: inherit; transition: border-color 0.15s, color 0.15s; display: flex; align-items: center; gap: 6px; }
        ...
        .buy-btn { flex: 1; min-width: 160px; padding: 13px 20px; border-radius: 10px; font-size: 14px; font-weight: 600; text-decoration: none; text-align: center; display: block; transition: opacity 0.15s; }
      ...
      <div style={{ minHeight: '100vh', background: '#f7f9fc', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        <Nav variant="dashboard" rightContent={
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <a href={`/homes/${homeId}/rooms/${roomId}`} style={{ fontSize: '13px', color: '#6b7280', textDecoration: 'none', fontWeight: 500 }}>← Back to Room</a>
            <button className="edit-btn" onClick={() => window.location.href = `/homes/${homeId}/rooms/${roomId}/materials/${material.id}/edit`}>
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M9.5 1.5l3 3L4 13H1v-3L9.5 1.5z" stroke="currentColor" strokeWidth="1.3" fill="none" strokeLinejoin="round"/></svg>
              Edit
            </button>
          </div>
        } />
        ...
        <div style={{ maxWidth: '720px', margin: '0 auto', padding: '40px 40px 80px' }}>
          ...
          {material.photo_url ? (
            <div style={{ borderRadius: '20px', overflow: 'hidden', marginBottom: '24px' }}>
              <img src={material.photo_url} alt={material.name} style={{ width: '100%', maxHeight: '380px', objectFit: 'cover', display: 'block' }} />
            </div>
          ) : (
          ...
          <div style={{ background: '#0D1B2A', borderRadius: '16px', padding: '22px 24px', display: 'flex', alignItems: 'center', gap: '18px', border: '1px solid rgba(255,255,255,0.06)', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: 'rgba(61,184,90,0.15)', border: '1px solid rgba(61,184,90,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><circle cx="11" cy="11" r="4" stroke="#3db85a" strokeWidth="1.5" fill="none"/><path d="M11 2v2M11 18v2M2 11h2M18 11h2M4.22 4.22l1.42 1.42M16.36 16.36l1.42 1.42M4.22 17.78l1.42-1.42M16.36 5.64l1.42-1.42" stroke="#3db85a" strokeWidth="1.5" strokeLinecap="round"/></svg>
            </div>
            <div>
