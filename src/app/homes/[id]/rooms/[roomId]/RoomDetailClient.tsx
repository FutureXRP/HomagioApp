...
function RoomIcon({ type, size = 40 }: { type: string, size?: number }) {
  ...
  if (t.includes('bedroom')) return <svg width={size} height={size} viewBox="0 0 40 40" fill="none"><rect x="4" y="18" width="32" height="18" rx="3" stroke={stroke} strokeWidth="1.5" fill="none"/><path d="M4 26h32M12 26V18M28 26V18" stroke={stroke} strokeWidth="1.5"/></svg>
  if (t.includes('bathroom')) return <svg width={size} height={size} viewBox="0 0 40 40" fill="none"><path d="M8 22h24v8a6 6 0 01-6 6H14a6 6 0 01-6-6v-8z" stroke={stroke} strokeWidth="1.5" fill="none"/><path d="M8 22V10a4 4 0 018 0v12" stroke={stroke} strokeWidth="1.5"/></svg>
  if (t.includes('kitchen')) return <svg width={size} height={size} viewBox="0 0 40 40" fill="none"><rect x="5" y="8" width="30" height="24" rx="3" stroke={stroke} strokeWidth="1.5" fill="none"/><path d="M5 18h30M14 18v14M14 8v10" stroke={stroke} strokeWidth="1.5"/></svg>
  if (t.includes('exterior')) return <svg width={size} height={size} viewBox="0 0 40 40" fill="none"><path d="M5 19L20 6l15 13v17a2 2 0 01-2 2H7a2 2 0 01-2-2V19z" stroke={stroke} strokeWidth="1.5" fill="none"/><path d="M15 38V28h10v10" stroke={stroke} strokeWidth="1.5" fill="none"/></svg>
  if (t.includes('garage')) return <svg width={size} height={size} viewBox="0 0 40 40" fill="none"><rect x="4" y="14" width="32" height="22" rx="2" stroke={stroke} strokeWidth="1.5" fill="none"/><path d="M4 22h32M12 22v14M20 22v14M28 22v14M7 14L20 5l13 9" stroke={stroke} strokeWidth="1.5"/></svg>
  if (t.includes('office')) return <svg width={size} height={size} viewBox="0 0 40 40" fill="none"><rect x="8" y="6" width="24" height="20" rx="2" stroke={stroke} strokeWidth="1.5" fill="none"/><path d="M14 26v8M26 26v8M8 34h24" stroke={stroke} strokeWidth="1.5"/></svg>
  return <svg width={size} height={size} viewBox="0 0 40 40" fill="none"><rect x="4" y="4" width="32" height="32" rx="3" stroke={stroke} strokeWidth="1.5" fill="none"/><path d="M4 16h32M16 16v20" stroke={stroke} strokeWidth="1.5"/></svg>
}
          ...
          <div style={{ background: '#fff', borderRadius: '20px', border: '1px solid #e9edf2', overflow: 'hidden', marginBottom: '24px' }}>
            {roomPhoto ? (
              ...
              <div onClick={() => fileInputRef.current?.click()}
                style={{ height: '200px', background: roomColor, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', gap: '12px', position: 'relative', overflow: 'hidden' }}
              ...
              >
                ...
                {photoUploading
                  ...
                  : <><RoomIcon type={room.type} size={40} /><div style={{ fontSize: '14px', fontWeight: 600, color: 'rgba(255,255,255,0.6)' }}>Add a room photo</div></>}
              </div>
            )}
            ...
            <div style={{ padding: '28px 32px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
              ...
              <button onClick={() => window.location.href = `/homes/${homeId}/rooms/${roomId}/materials/add`}
                style={{ background: '#3db85a', color: '#fff', border: 'none', padding: '11px 22px', borderRadius: '9px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
          ...
          <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e9edf2', padding: '28px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
              <div>
                ...
                <div style={{ fontSize: '13px', color: '#9ca3af', marginTop: '2px' }}>{materials.length} item{materials.length !== 1 ? 's' : ''} — click any to view details</div>
              </div>
              {materials.length > 0 && (
                <button onClick={() => window.location.href = `/homes/${homeId}/rooms/${roomId}/materials/add`}
            ...
            {materials.length === 0 ? (
              <div style={{ background: '#0D1B2A', borderRadius: '12px', padding: '48px 32px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
                ...
                <button onClick={() => window.location.href = `/homes/${homeId}/rooms/${roomId}/materials/add`}
                  ...
                  onMouseEnter={e => (e.currentTarget.style.background = '#34a34f')}
                  onMouseLeave={e => (e.currentTarget.style.background = '#3db85a')}
                >Add Your First Material</button>
              </div>
            ) : (
