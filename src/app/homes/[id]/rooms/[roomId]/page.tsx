          ...
          <div style={{display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px'}}>
            <div style={{display: 'flex', alignItems: 'center', gap: '16px'}}>
              ...
              <div>
                ...
                <p style={{fontSize: '15px', color: '#888', marginTop: '4px'}}>{room.type}</p>
                {room.notes && <p style={{fontSize: '14px', color: '#666', marginTop: '8px'}}>{room.notes}</p>}
              </div>
            </div>
            <button onClick={() => window.location.href = `/homes/${params.id}/rooms/${params.roomId}/materials/add`}
              ...
              {materials.map(material => (
                <div key={material.id} style={{display: 'flex', alignItems: 'center', gap: '14px', padding: '14px', border: '1px solid #f0f0f0', borderRadius: '10px'}}>
                  ...
                  <div style={{flex: 1}}>
                    ...
                    <div style={{fontSize: '12px', color: '#888', marginTop: '2px'}}>{[material.brand, material.color, material.finish].filter(Boolean).join(' · ') || 'No details'}</div>
                  </div>
                  {material.cost > 0 && <div style={{fontSize: '14px', fontWeight: 600, color: '#006aff', flexShrink: 0}}>${(material.cost/100).toLocaleString()}</div>}
                </div>
              ))}
            </div>
          )}
