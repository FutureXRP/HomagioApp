...
export default function PublicRoomClient({ home, room, materials }: { home: any, room: any, materials: any[] }) {
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .material-row { display: flex; align-items: center; gap: 14px; padding: 14px 16px; border-radius: 12px; border: 1.5px solid #e9edf2; background: #fff; cursor: pointer; text-decoration: none; transition: border-color 0.15s, box-shadow 0.15s, transform 0.15s; }
        .material-row:hover { border-color: #3db85a; box-shadow: 0 4px 16px rgba(61,184,90,0.1); transform: translateY(-1px); }
      ...
      <div style={{ minHeight: '100vh', background: '#f7f9fc', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        ...
        <div style={{ maxWidth: '860px', margin: '0 auto', padding: '40px 40px 80px' }}>
          ...
          <div style={{ marginBottom: '40px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
              <div>
                ...
                <p style={{ fontSize: '13px', color: '#9ca3af', marginTop: '3px' }}>{materials.length} item{materials.length !== 1 ? 's' : ''} cataloged</p>
              </div>
            </div>
            {materials.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {materials.map(material => {
                  ...
                  return (
                    <a key={material.id} href={`/explore/${home.id}/rooms/${room.id}/materials/${material.id}`} className="material-row">
          ...
          <div style={{ background: '#0D1B2A', borderRadius: '20px', padding: '40px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ fontSize: '14px', color: 'rgba(255,255,255,0.45)', marginBottom: '24px', lineHeight: 1.6 }}>Track every material, finish, and fixture — just like this room.</div>
