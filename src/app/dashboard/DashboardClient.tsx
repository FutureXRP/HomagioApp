...
const FEATURES = [
  { title: 'Catalogue My Home', desc: 'Add rooms, materials, and track every detail of your home.', href: '/dashboard/homes', category: 'My Homes', active: true, cta: 'Get started →' },
  { title: 'Explore Homes Near Me', desc: 'Discover how neighbors designed their homes and get inspired.', href: '/explore', category: 'Explore', active: true, cta: 'Explore now →' },
  { title: 'Find a Material', desc: 'Search homes by flooring, countertop, paint color, and more.', href: '#', category: 'Search', active: false },
  { title: 'Identify a Material', desc: 'Upload a photo and our AI will tell you exactly what it is.', href: '#', category: 'AI Detection', active: false },
  { title: 'Homagio Estimate™', desc: "See your home's estimated value and track it over time.", href: '#', category: 'Estimate', active: false },
  { title: 'Find a Pro', desc: 'Connect with designers, builders, and realtors in your area.', href: '#', category: 'Pro Studio', active: false },
]
...
export default function DashboardClient({ user, homes, homesCount, roomsCount, materialsCount, totalValue, recentMaterials }: Props) {
  const firstName = user?.user_metadata?.full_name
    ? user.user_metadata.full_name.split(' ')[0]
    : user?.email?.split('@')[0] || 'there'

  const isNewUser = homesCount === 0
  const formattedValue = totalValue > 0 ? `$${Math.round(totalValue / 100).toLocaleString()}` : '$0'
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .feature-card { background: #0D1B2A; border-radius: 16px; overflow: hidden; position: relative; min-height: 210px; display: flex; flex-direction: column; justify-content: flex-end; border: 1px solid rgba(255,255,255,0.06); transition: transform 0.2s ease, box-shadow 0.2s ease, border-color 0.2s ease; }
        .feature-card.active { cursor: pointer; }
        .feature-card.active:hover { transform: translateY(-4px); box-shadow: 0 16px 48px rgba(0,0,0,0.3); border-color: rgba(61,184,90,0.3); }
        .feature-card.inactive { opacity: 0.6; cursor: default; }
        .card-shine { position: absolute; top: 0; left: 0; right: 0; height: 1px; background: linear-gradient(90deg, transparent, rgba(255,255,255,0.08), transparent); }
        .card-soon { position: absolute; top: 16px; right: 16px; background: rgba(255,255,255,0.07); border: 1px solid rgba(255,255,255,0.1); border-radius: 20px; padding: 3px 10px; font-size: 10px; font-weight: 600; color: rgba(255,255,255,0.4); letter-spacing: 0.06em; }
        .home-card { background: #fff; border: 1.5px solid #e9edf2; border-radius: 16px; overflow: hidden; cursor: pointer; transition: border-color 0.15s, box-shadow 0.15s, transform 0.15s; }
        .home-card:hover { border-color: #3db85a; box-shadow: 0 6px 24px rgba(61,184,90,0.12); transform: translateY(-2px); }
        ...
        .material-row { display: flex; align-items: center; gap: 14px; padding: 14px 16px; border-radius: 10px; border: 1px solid #f3f4f6; cursor: pointer; transition: background 0.12s, border-color 0.12s; }
        .material-row:hover { background: #f7f9fc; border-color: #e2e8f0; }
        .quick-link { display: flex; align-items: center; padding: 9px 18px; border-radius: 8px; border: 1.5px solid #e2e8f0; background: #fff; font-size: 13px; font-weight: 500; color: #374151; text-decoration: none; font-family: inherit; transition: border-color 0.15s, color 0.15s, background 0.15s; white-space: nowrap; }
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '52px 40px 80px' }}>
          ...
          <div style={{ marginBottom: '40px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              ...
              <h1 style={{ fontSize: '34px', fontWeight: 700, color: '#1a1a2e', letterSpacing: '-0.75px', marginBottom: '8px', lineHeight: 1.2 }}>Welcome back, {firstName}</h1>
                  ...
                  <div key={home.id} className="home-card" onClick={() => window.location.href = `/homes/${home.id}`}>
                    <div style={{ height: '140px', background: '#0D1B2A', overflow: 'hidden', position: 'relative' }}>
                      {home.photo_url
                        ...
                        : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><svg width="48" height="48" viewBox="0 0 48 48" fill="none" opacity="0.3"><path d="M6 23L24 7l18 16V43a2 2 0 01-2 2H8a2 2 0 01-2-2V23z" stroke="#fff" strokeWidth="2" fill="none"/><path d="M18 45V33h12v12" stroke="#fff" strokeWidth="2" fill="none"/></svg></div>}
                      {home.is_public && <div style={{ position: 'absolute', top: '10px', right: '10px', background: 'rgba(61,184,90,0.9)', borderRadius: '20px', padding: '2px 8px', fontSize: '10px', fontWeight: 600, color: '#fff' }}>PUBLIC</div>}
                    </div>
                    <div style={{ padding: '16px 18px' }}>
                      <div style={{ fontSize: '15px', fontWeight: 700, color: '#1a1a2e', marginBottom: '3px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{home.name || home.address}</div>
          ...
          <div style={{ marginBottom: '40px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px' }}>
              {FEATURES.map(feature => (
                <div key={feature.title} className={`feature-card ${feature.active ? 'active' : 'inactive'}`} onClick={() => feature.active && (window.location.href = feature.href)}>
            ...
            <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e9edf2', padding: '28px', marginBottom: '40px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {recentMaterials.map(material => (
                  <div key={material.id} className="material-row" onClick={() => window.location.href = `/homes/${material.home_id}/rooms/${material.room_id}/materials/${material.id}`}>
          ...
          <div style={{ background: '#fff', border: '1px solid #e9edf2', borderRadius: '14px', padding: '18px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {[{ label: 'My Homes', href: '/dashboard/homes' }, { label: 'Explore', href: '/explore' }, { label: 'Add Home', href: '/homes/add' }, { label: 'Settings', href: '#' }].map(item => (
