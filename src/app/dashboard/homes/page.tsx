      ...
      <div style={{padding: '18px 20px', flex: 1}}>
        ...
        {hasDetails && (
          <div style={{display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '8px', paddingTop: '12px', borderTop: '1px solid #f3f4f6'}}>
            ...
            {year && <span style={{fontSize: '11px', color: '#6b7280'}}>📅 {year}</span>}
          </div>
        )}
      </div>
    </div>
...
export default function HomesDashboard() {
  ...
  useEffect(() => {
    ...
    const loadHomes = async (userId: string) => {
      ...
      if (homesError) setError('Could not load homes: ' + homesError.message)
      setHomes(homesData || [])
      setLoading(false)
    }
    ...
    const init = async () => {
      ...
      // Session found — load immediately
      if (session?.user) {
        setUser(session.user)
        await loadHomes(session.user.id)
        return
      }
      // No session yet on first call — do NOT redirect here.
      // The session cookie may not have hydrated yet on the client.
      // onAuthStateChange below will fire INITIAL_SESSION shortly and handle it.
    }
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (session?.user) {
        // Session resolved — load homes if not already loaded
        setUser(session.user)
        await loadHomes(session.user.id)
