    ...
    >
      <div style={{ height: '160px', background: '#0D1B2A', overflow: 'hidden', flexShrink: 0, position: 'relative' }}>
        {home.photo_url
          ...
          : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><svg width="52" height="52" viewBox="0 0 52 52" fill="none" opacity="0.2"><path d="M6 25L26 7l20 18V47a2 2 0 01-2 2H8a2 2 0 01-2-2V25z" stroke="#fff" strokeWidth="2" fill="none"/><path d="M19 49V35h14v14" stroke="#fff" strokeWidth="2" fill="none"/></svg></div>}
        {home.is_public && <div style={{ position: 'absolute', top: '10px', right: '10px', background: 'rgba(61,184,90,0.9)', borderRadius: '20px', padding: '3px 9px', fontSize: '10px', fontWeight: 700, color: '#fff' }}>PUBLIC</div>}
      </div>
      <div style={{ padding: '18px 20px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: hasDetails ? '12px' : '0' }}>{home.city}, {home.state} {home.zip}</div>
        {hasDetails && (
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .add-home-card { background: #fff; border: 2px dashed #e9edf2; border-radius: 16px; display: flex; align-items: center; justify-content: center; cursor: pointer; min-height: 240px; flex-direction: column; gap: 10px; transition: border-color 0.15s, background 0.15s; }
        .add-home-card:hover { border-color: #3db85a; background: #f0fdf4; }
        .add-home-card:hover .add-icon { background: #3db85a; color: #fff; }
        .add-icon { width: 44px; height: 44px; border-radius: 50%; background: #f3f4f6; display: flex; align-items: center; justify-content: center; font-size: 20px; color: #9ca3af; transition: background 0.15s, color 0.15s; }
      `}</style>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '52px 40px 80px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '40px', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              ...
              <p style={{ fontSize: '14px', color: '#6b7280' }}>{homes.length === 0 ? 'Add your first home to get started' : `${homes.length} home${homes.length > 1 ? 's' : ''} cataloged`}</p>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px', marginBottom: '40px' }}>
            {[
              { label: 'Homes', value: homes.length, svg: <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M3 9.5L10 3l7 6.5V17a1 1 0 01-1 1H4a1 1 0 01-1-1V9.5z" stroke="#3db85a" strokeWidth="1.5" fill="none"/><path d="M7 18v-5h6v5" stroke="#3db85a" strokeWidth="1.5" fill="none"/></svg> },
              { label: 'Public Homes', value: homes.filter(h => h.is_public).length, svg: <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><circle cx="10" cy="10" r="7.5" stroke="#3db85a" strokeWidth="1.5" fill="none"/><path d="M10 2.5c0 0-3 3-3 7.5s3 7.5 3 7.5M10 2.5c0 0 3 3 3 7.5s-3 7.5-3 7.5M2.5 10h15" stroke="#3db85a" strokeWidth="1.5"/></svg> },
              { label: 'Total Sq Ft', value: homes.reduce((sum, h) => sum + (h.square_feet || 0), 0).toLocaleString() || '—', svg: <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><rect x="2.5" y="2.5" width="15" height="15" rx="1.5" stroke="#3db85a" strokeWidth="1.5" fill="none"/><path d="M6 14l3-3 2 2 3-4" stroke="#3db85a" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg> },
              { label: 'Cities', value: Array.from(new Set(homes.map(h => h.city).filter(Boolean))).length || '—', svg: <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M10 2a5 5 0 015 5c0 3.5-5 11-5 11S5 10.5 5 7a5 5 0 015-5z" stroke="#3db85a" strokeWidth="1.5" fill="none"/><circle cx="10" cy="7" r="1.5" stroke="#3db85a" strokeWidth="1.5" fill="none"/></svg> },
            ].map(stat => (
          ...
          {homes.length === 0 ? (
            <div style={{ background: '#0D1B2A', borderRadius: '20px', padding: '80px 32px', textAlign: 'center', position: 'relative', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '20px' }}><svg width="56" height="56" viewBox="0 0 56 56" fill="none" opacity="0.4"><path d="M7 27L28 8l21 19V50a2 2 0 01-2 2H9a2 2 0 01-2-2V27z" stroke="#fff" strokeWidth="2" fill="none"/><path d="M21 52V38h14v14" stroke="#fff" strokeWidth="2" fill="none"/></svg></div>
              <h2 style={{ fontSize: '22px', fontWeight: 700, color: '#fff', marginBottom: '10px', letterSpacing: '-0.5px' }}>Add your first home</h2>
              <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.5)', maxWidth: '360px', margin: '0 auto 32px', lineHeight: 1.7 }}>Start building your home's digital record. Catalog every material, track every dollar.</p>
              <button onClick={() => window.location.href = '/homes/add'}
